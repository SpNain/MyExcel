...
function updateChildrens(childrens) {
    for (let i = 0; i < childrens.length; i++) { // ye as a base case bhi kaam krega kyunki agr childeren honge hi nhi to unki length 0 hogi aur ye code chalega hi nhi
        ...
        let updatedValueOfChild = solveFormula(childCellObject.formula); // B1 ko jis formula ke hisab se solve karna hai, maanlo kar liya, value aa gyi 30
        //db update
        ...
        //recursive call
        updateChildrens(childCellObject.childrens); // agr child ke bhi aage children honge to unko recursively call lga ke smbhal liya hai  // DFS type of code
    }
}